...
- The resulting `trashwheel_df` dataset is a combination of 3 datasets
  *Mr. Trash Wheel*, *Professor Trash Wheel*, and *Gwynnda Trash Wheel*
  . In total, the `trashwheel_df` dataset contains845 observations of 15
  variables, among which the `name` variable represent the trash wheel’s
  name. The resulting dataset contains 584 observations from *Mr. Trash
  Wheel* dataset, 106 observations from *Professor Trash Wheel* dataset,
  and 155 observations from *Gwynnda Trash Wheel* dataset. The resulting
  ...
  `glass_bottles`(which is not collected by Gwynnda Trash Wheel),
  `plastic_bags`, `wrappers`, `sports_balls` (which is only collected by
  Mr.Trash Wheel). It also contains the (approximate) number of homes
...
- To clean and tidy the dataset of baseline demographics, I first
  ...
  more appropriate to read. Finally, I use `filter` function to remove
  any participants who do not meet the stated inclusion criteria
  (i.e. no MCI at baseline), which is either `current_age` less than
  `age_at_onset` or with “.” in `age_at_onset`.
...
- A: The average baseline age is 65.0286013.
...
- To clean and tidy the dataset of f longitudinally observed biomarker
  ...
  convenience afterwards. Next, I changed the 5 variables(`baseline`,
  `time2`,`time4` ,`time6`,`time8`) measuring the amyloid 42/40 ratio at
  difference times since the study baseline to two variables measuring
  `time_elapsed` and `amyloid_ratio`. For convenience, I change
  “baseline” to 0 to align with other representation of specific time.
  Because the original data contains an observation with `Na` in
  `baseline`, I change it to `NA` for later convenience. Finally, I
  change the column name `study id` to `id` for convenient match of the
  other dataset afterwards.
- After cleaning, the result dataset contains 2435 observations of 3
  variables.
...
- There are 8 participants only appear in the baseline dataset (these
  participants are `id_baseline_only` with id 4,49,92,179,268,304,389
  and 412), while there are 16 participants(one id appears five times)
  participants only appear in the amyloid datase (these participants are
  `id_amyloid_only` with id
...
- The resulting dataset `both_joined_df` contains the data of 2355
  observations, 471 participants(because we tidy the `amyloid_df`to make
  each participant with 5 observations related with `time_elapsed`,each
  id appears 5 times) that appear in both datasets, and the observation
  of 8 variables. These variables including the basic demographic
  information of the participants
  (their`sex`,`current_age`,`education`). Their age of MCI onset
  `age_at_onset` and whether they are apoe4 carrier `apoe4`. It also
  includes the time (in years) elapsed since the study baseline to the
  visit where biomarker amyloid 42/40 ratio was measured: `baseline`,
  `time2`,`time4` ,`time6`,`time8`, and the ratio measured
  `amyloid_ratio`.
- There are 205 participants who are female, and 266 participants who
  are male.
- There are 142 participants who are APOE4 carrier, and 329 participants
  who are APOE4 non-carrier.
- There are 90 participants who develop MCI
